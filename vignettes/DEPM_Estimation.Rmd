...
* Female total weight and batch fecundities to estimate mean female fecunity (*F*)
...
* Z - a pre-specified level of egg mortality used to determine $P_{0}$. Multiple values can be provided to simultaneously determine different values of $P_{0}$ based on this parameter. Instruction on how to do this and how to performed sensitivity tests are provided in the final section of this vignette.
...
approach, the *W* parameter is used and represents the mean female weight its variance. Similarly, fecundity is also expressed as the mean number of eggs per adult female and its variance.

**Fecundity**
Fecundity needs to be summarised in a couple of ways. Firstly, a batch-fecundity-at-weight relationship is required for **gonad free weight** which can be used to predict the fecundity of fish that whose histology has not been examined. This relationship can be estimated using the `Estimate_Batch_Fecundity()` function. This function is quite useful as it can return three sets of outputs:
